...
class BNO08X:
    ...
    def __init__(self, _interface, reset_pin=None, int_pin=None, cs_pin=None, wake_pin=None, debug=False) -> None:
        ...
        else:# 【追加】INTピンがない場合、タイマーでフラグを立てる
            self.polling_timer = Timer(-1)
            # 20ms（50Hz）ごとにフラグをTrueにする
            self.polling_timer.init(period=20, mode=Timer.PERIODIC, callback=self._fake_interrupt)
    ...
    def _fake_interrupt(self, timer):
        # 割り込みハンドラと同じフラグを立てる
    ...
    def _process_control_report(self, report_id: int, report_bytes: bytearray) -> None:
        ...
        if report_id == _REPORT_PRODUCT_ID_RESPONSE:
            ...
            if not self._product_id_received:
                if reset_cause != 2 and reset_cause != 4:
