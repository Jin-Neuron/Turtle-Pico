...
class BNO08X_I2C(BNO08X):
    ...
    def __init__(self, i2c_bus, address=_BNO08X_DEFAULT_ADDRESS, reset_pin=None, int_pin=None, debug=False):
        ...
        if int_pin is not None and not isinstance(int_pin, Pin):
    ...
    def _wait_for_int(self, timeout_us=1000):
        if self._int_pin is None:
            raise RuntimeError("_wait_for_int called but no int_pin defined")
    ...
    def _read_packet(self, wait=False):
        ...
        if len(self._assembly_buffer) + 4 < self._target_len:
            ...
            if self._int_pin is not None:
                # INTピンがある場合は割り込みを待つ
                data_ready = self._wait_for_int(timeout_us=10000)
            else:
                # 【改造】ポーリングモード：I2Cでセンサーが応答するまで待つ
                import time
                start = time.ticks_ms()
                while time.ticks_diff(time.ticks_ms(), start) < 50: # 50msタイムアウト
                    try:
                        # 0バイト書き込みでACKが返るか（準備できたか）確認
                        i2c.writeto(i2c_addr, b'')
                        data_ready = True
                        break
                    except OSError:
                        pass # まだ準備中（NACK）
